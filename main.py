...
def add_marks(student_id: int, marks: MarksBase, db: Session = Depends(get_db)):
    student = db.query(StudentData).filter(StudentData.id == student_id).first()
    if not student:
...
@app.put("/student/{student_id}", response_model=ResponseModel)
def update_student(student_id: int, data: CreateBase, db: Session = Depends(get_db)):
    s = db.query(StudentData).filter(StudentData.id == student_id).first()
    if not s:
        raise HTTPException(404, "Student not found")

    s.name = data.name
    s.school_class = data.school_class
    s.section = data.section
    s.door_no = data.door_no
    s.address = data.address
    s.city = data.city
    s.phone_no = data.phone_no
    s.blood_group = data.blood_group
    s.father_name = data.father_name
    s.mother_name = data.mother_name
    s.yearly_occupation = data.yearly_occupation

    db.commit()
    db.refresh(s)
    return s

@app.put("/student/{student_id}/marks", response_model=MarksResponse)
def update_marks(student_id: int, marks: MarksBase, db: Session = Depends(get_db)):
    m = db.query(StudentMarks).filter(StudentMarks.student_id == student_id).first()
    if not m:
        raise HTTPException(404, "Marks not found")

    m.tamil_mark = marks.tamil_mark
    m.english_mark = marks.english_mark
    m.maths_mark = marks.maths_mark
    m.science_mark = marks.science_mark
    m.social_mark = marks.social_mark
